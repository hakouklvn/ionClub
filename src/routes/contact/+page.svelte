<script>
	...
	import { Button, Spinner, Input, Label, Textarea, Alert } from 'flowbite-svelte';
...
<div class="lg:flex gap-3 lg:border lg:rounded-lg lg:border-grey-300">
	<form
		class="w-full flex flex-col lg:mt-8 lg:p-16 mt-2 p-4 space-y-6"
		method="post"
		use:enhance={() => {
			loadingModal = true;
			return async ({ update }) => {
		...
		{:else}
			<button
				class="bg-orange-500 hover:bg-orange-700 text-white font-bold py-2 px-4 w-full rounded"
			>
				Send your message
			</button>
		{/if}
